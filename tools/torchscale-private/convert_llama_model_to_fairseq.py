...
def load_checkpoint(checkpoint_path):
    ...
    keys_to_rename = {
        "embed_tokens.weight": "decoder.embed_tokens.weight",
    ...
    logger.warning("Fairseq Model are assumed to have Shared Input-Output Projection!")
    sd["decoder.output_projection.weight"] = sd["decoder.embed_tokens.weight"]

    keys = list(sd.keys())
    for key in keys:
        if ".mlp" in key:
            value = sd[key]
            new_name = key.replace(".mlp", ".ffn")
        ...
        if ".o_proj" in key:
            value = sd[key]
            new_name = key.replace(".o_proj", ".out_proj")
            sd[new_name] = value
            del sd[key]

        if ".input_layernorm" in key:
            value = sd[key]
            new_name = key.replace(".input_layernorm", ".self_attn_layer_norm")
            sd[new_name] = value
            del sd[key]

        if ".post_attention_layernorm" in key:
            value = sd[key]
            new_name = key.replace(".post_attention_layernorm", ".final_layer_norm")
            sd[new_name] = value
            del sd[key]
