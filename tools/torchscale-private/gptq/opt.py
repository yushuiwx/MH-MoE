...
def get_opt(model, load_path="", spm_model=""):
...
def opt_eval(model, testenc, dev):
    ...
    for i in range(nsamples):
        ...
        batch = testenc[:, (i * model.seqlen):((i + 1) * model.seqlen)].to(dev)
        lm_logits = model(batch, labels=batch, use_cache=False).logits
        ...
        # # loss_fct = nn.CrossEntropyLoss()
        # # loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1).to(dev))
...
def opt_pack3(model, quantizers):
    ...
    for name in qlayers:
        ...
        qlayers[name].pack(layers[name], quantizers[name].scale.cpu(), quantizers[name].zero.cpu())
    print('Done.')
    return model
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--spm-model', type=str, default='',
        help='SentencePiece model'
    )
    parser.add_argument(
        '--dict-path', type=str, default='',
        help=''
    )
    ...
    if args.load:
        ...
        model = get_opt(args.model, spm_model=args.spm_model)
        model.eval()

    if args.wbits < 16 and not args.nearest:
        dataloader, testloader = get_loaders(
            args.dataset, nsamples=args.nsamples, seed=args.seed, model=args.model, seqlen=model.seqlen,
            spm_model=args.spm_model, dict_path=args.dict_path,
        )
        tick = time.time()
    ...
    for dataset in datasets: 
        dataloader, testloader = get_loaders(
            ...
            spm_model=args.spm_model, dict_path=args.dict_path, nsamples=args.nsamples
        )
        print(dataset)
    ...
    if args.save:
        if args.spm_model == "":
            # do not perform packing
            opt_pack3(model, quantizers)
        Path(args.save).mkdir(exist_ok=True)
        # save OPTModel
        model.model.save_pretrained(args.save, max_shard_size="1000GB")
